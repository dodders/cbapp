...
var initState = {"body": {"_items": [], "_links": {}, "_meta": {}}}
...
class App extends React.Component {
	constructor(props){
		...
		request(url, function(err, resp, body) {
			...
			me.setState({"body": JSON.parse(body)});
		});
	}
	...
	render () {
		...
		// 	var d = new Date(0);
		// 	var point = {"date": d.setUTCMilliseconds(body._items[i].time * 1000), "value": i}
		// 	sensorData.push(point);
		// }
		console.log(body)
		console.log(body._items.length, ' items found.')
		var curData = [] //array of objects {'date': dateObj, 'value': valueObj}
		//body._items.forEach(function(el) {
		// 	curData.push({'date': this.fmtDate(el.time), 'value': this.fmtTemp(el.value)});
		//}, this);
		if (body._items.length != 0) {
			for (var i = 0; i < 10; i++) {
		...
		var datedata = MetricsGraphics.convert.date(curData, "date", "%m/%d/%y")
